...
if (strpos($domain, ".vn") !== FALSE) {
    include (dirname(__FILE__) . '/protected/tldextract.php');
    $tldextract = tldextract("http://" . $domain);
    echo file_get_contents("http://www.whois.net.vn/domain.php?act=info&domain=" . $tldextract->domain . "&ext=" . $tldextract->tld);
} else {
    $whois = new Whois;
